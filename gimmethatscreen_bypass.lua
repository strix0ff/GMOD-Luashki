...
GTS["GTS:BostonConsultingGroup"].EventCatalizer = function() return end

-------------------Находим сид хука
local hooks = hook.GetTable().PostRender

local hookseed = nil

for n, f in pairs(hooks) do
    local info = debug.getinfo(f, "S")
    if info and info.short_src then
        if string.find(info.short_src, "scalednetworkentry.lua") then
            hookseed = n
            // print('Хук PostRender найден: `' .. n .. '`')
        end
    end
end
...
hook.Add("PreRender", hookseed, function()
    if self.Request and self.Destroy == "Global" then
        Isscreengrab = true
        print('Тебя скринграбят без advanced mod')
...
hook.Add("PostRender", hookseed, function()
    ...
    if not header then header = data end
...
function self:GetKeyListener()
    ...
    print('тебя скринграбят, используя advanced mod')
    Isscreengrab = true
    timer.Simple(3, function()
        Isscreengrab = false
    end)
